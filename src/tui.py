...
class TUIApp:
    ...
    def run(self):
        """Run the TUI with error handling."""
        try:
            ...
            # Show error in a curses-safe way
            try:
                curses.wrapper(self._show_error, str(e))
            except:
                # Curses failed, print to stderr
                print(f"\n{'='*60}", file=sys.stderr)
                print(f"ERROR: {e}", file=sys.stderr)
                if hasattr(e, 'fix'):
                    print(f"\n💡 How to fix: {e.fix}", file=sys.stderr)
                print(f"{'='*60}\n", file=sys.stderr)
                sys.exit(1)
    
    def _show_error(self, stdscr, error_msg):
        """Display an error message in curses and wait for user to exit."""
        stdscr.clear()
        h, w = stdscr.getmaxyx()
        
        # Format error message
        lines = ["ERROR:", ""] + error_msg.split('\n')
        lines.append("")
        ...
        win.box()
        win.addstr(0, 2, " ERROR ", curses.A_REVERSE | curses.A_BOLD)
        
        for i, line in enumerate(lines[:box_h-2]):
            if i + 1 >= box_h - 1: break
            display = line[:box_w-4] if len(line) > box_w-4 else line
            win.addstr(i + 1, 2, display)

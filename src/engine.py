...
from .exceptions import NimNotFoundError, LessonNotFoundError, ConfigError
...
class Engine:
    ...
    def check_nim_installed(self):
        """Raises NimNotFoundError if 'nim' command is not found."""
        try:
            subprocess.run(
                ["nim", "--version"],
                check=True,
        ...
        try:
            with open(self.progress_file, "r", encoding="utf-8") as f:
                ...
                if not isinstance(data, list):
                    raise ConfigError(str(self.progress_file), "invalid format (expected list)")
                return set(data)
        except json.JSONDecodeError:
            raise ConfigError(str(self.progress_file), "corrupted (invalid JSON)")
        except IOError as e:
            raise ConfigError(str(self.progress_file), f"unreadable ({e})")
    ...
    def save_progress(self, completed_ids: set):
        ...
        try:
            ...
            raise ConfigError(str(self.progress_file), f"unwritable ({e})")
        ...
        try:
            with open(self.state_file, "r", encoding="utf-8") as f:
                ...
                if not isinstance(data, dict):
                    raise ConfigError(str(self.state_file), "invalid format (expected dict)")
                return data
        except json.JSONDecodeError:
            raise ConfigError(str(self.state_file), "corrupted (invalid JSON)")
        except IOError as e:
            raise ConfigError(str(self.state_file), f"unreadable ({e})")
    ...
    def get_lesson_by_id(self, lesson_id: str) -> Dict[str, Any]:
        """Finds a lesson by its ID. Raises LessonNotFoundError if not found."""
